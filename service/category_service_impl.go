...
func (service *CategoryServiceImpl) Create(ctx context.Context, request web.CategoryCreateRequest) web.CategoryResponse {
	...
	// (4) Create new object category
	category := domain.Category{
		// Set name from request
		Name: request.Name,
	}

	// (5) Save transaction with use Repository
	category = service.CategoryRepository.Save(ctx, tx, category)

	// (6) Return after success
...
func (service *CategoryServiceImpl) Update(ctx context.Context, request web.CategoryUpdateRequest) web.CategoryResponse {
	...
	// (2) Find category in dataabase
	category, err := service.CategoryRepository.FindById(ctx, tx, request.Id)

	// (3) If error / category not found handle error with helper
	helper.PanicErr(err)

	// (4) If no error, set request name to object category
	category.Name = request.Name

	// (5) Update category with use Repository
	category = service.CategoryRepository.Update(ctx, tx, category)

	// (4) Return response with helper
	return helper.ToCategoryResponse(category)
}
